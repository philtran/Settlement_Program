...
text(idvec1[which(abs(S1)==max(abs(S1)))[1]],0.95*max(abs(S1*12)),paste0("Si=",format(round(max(abs(S1*12)),2),nsmall=2)," inches"))
...
legend("topleft",c("Allowable Settlement","Passed","Failed"),col=c("red","blue","red"),lty=c(2,NA,NA),pch=c(NA,1,1),lwd=c(2,NA,NA))
text(N-2,1.05*(Smax*12),paste0("Smax=",format(round(Smax*12,2),nsmall=2)," inches"))
text(idvec2[which(abs(S2)==max(abs(S2)))[1]],0.95*max(abs(S2*12)),paste0("Si=",format(round(max(abs(S2*12)),2),nsmall=2)," inches"))
